...
class MostAvailableScheduler(Scheduler):
    ...
    def schedule(self, pod: Pod) -> Optional[Node]:
        ...
        for node in self.cluster.get_nodes():
            if node.can_schedule_pod(pod.resources):
                diff_resources = node.resources_available.subtract(pod.resources)

                distance = 0
                for metric in diff_resources.metrics():
                    if pod.resources.get(metric, 0) > 0:  # Only consider metrics that the pod actually requests
                        distance += diff_resources.get(metric) ** 2
