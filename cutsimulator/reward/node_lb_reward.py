...
from cutsimulator.evaluation.evaluation_metrics import node_local_load_balance
from cutsimulator.reward.reward import BaseReward
from cutsimulator.workload.pod import Pod

# Cooperative Node-local Load Balancing Reward (all agents get same reward)
class Node_LB_reward(BaseReward):
    ...
    def compute(self, pod: Optional[Pod], selected_node: Optional[Node], valid_nodes: List[Node]) -> List[float]:
        ...
        # Average node-local balance (higher is better)
        reward = node_local_load_balance(all_nodes)
