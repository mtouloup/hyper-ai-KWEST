...
from cutsimulator.evaluation.evaluation_metrics import cluster_wide_load_balance, node_local_load_balance
from cutsimulator.reward.reward import BaseReward
from cutsimulator.workload.pod import Pod


class Coop_LB_reward(BaseReward): # Cooperative Load Balancing Reward
    ...
    def compute(self, pod: Optional[Pod], selected_node: Optional[Node], valid_nodes: List[Node]) -> List[float]:
        ...
        # Cluster-wide load balance + average node-local balance.
        # (Higher is better; values are bounded to [0, 1].)
        cluster_load_score = cluster_wide_load_balance(all_nodes)
        avg_node_balance = node_local_load_balance(all_nodes)

        reward = cluster_load_score + avg_node_balance
